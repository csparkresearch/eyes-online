...
def addScript():
	try:
		if session.get('user'):
			...
			newSnippet = UserScript(_user, _title,_description)
			try:
...
def updateScript():
  if session.get('user'):
    ...
    try:
      script = UserScript.query.filter_by(user=_user,id=_script_id).first()
      script.title = _title
...
def getScriptList():
	try:
		if session.get('user'):
			_user = session.get('user')[1]
			scripts = UserScript.query.filter_by(user=_user)
			scripts_dict = []
